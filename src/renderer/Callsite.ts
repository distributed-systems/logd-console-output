...
export class CallsiteRenderer extends Renderer {
    ...
    render({
        ...
        context.print(this.decorate(context, this.pad(this.truncateLeft(this.truncatePath(value.fileName.replace('file://', '') || 'n/a')), 31, ' '), 'path'));
    ...
    getSignature(callsite: ICallsite) {
        let signature = '';
